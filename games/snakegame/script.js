...
const AuthSystem = {
    ...
    loginOrRegister: async function(nome, idade, email) {
        ...
        const response = await fetch('../../backend/cadastro.php', {
            ...
            body: `nome=${encodeURIComponent(nome)}&idade=${encodeURIComponent(idade)}&email=${encodeURIComponent(email)}`
        });
        const data = await response.json();
...
function updateRanking(score, mode) {
  ...
  fetch('../../backend/update_ranking.php', {
    ...
    body: `user_id=${encodeURIComponent(user.id)}&score=${encodeURIComponent(score)}&mode=${encodeURIComponent(mode)}`
  })
...
async function getRanking(mode) {
    try {
        const response = await fetch(`../../backend/mostrar_ranking.php?mode=${encodeURIComponent(mode)}`);
...
const checkCollision = () => {
    ...
    const selfCollision = snake.length > 1 && snake.slice(0, -1).some(pos => pos.x === head.x && pos.y === head.y);
...
const gameLoop = () => {
    ...
    checkEat();
    checkCollision();
    if (!isGameOver) { // Só agenda o próximo loop se o jogo não terminou
